      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-slate-600 rounded-lg bg-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
          />
        </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 ${
                filter === status
                  ...
                  : 'bg-slate-700 text-slate-300 hover:bg-slate-600 hover:shadow-sm'
              }`}
            >
      ...
      <div className="grid gap-4">
        {filteredSubmissions.map((submission) => (
          <motion.div
            ...
            className="bg-slate-800 rounded-xl shadow-lg border border-slate-700 p-6"
          >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      {submission.partnershipType.slice(0, 3).map((type, index) => (
                        <span
                          key={index}
                          className="px-2 py-1 bg-emerald-900/30 text-emerald-400 text-xs rounded-full"
                        >
                          {type}
                        </span>
                      ))}
      ...
      {selectedSubmission && (
        ...
        >
          <motion.div
            ...
            className="bg-slate-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-slate-700"
          ...
          >
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  onClick={() => setSelectedSubmission(null)}
                  className="text-slate-400 hover:text-slate-200 p-2 hover:bg-slate-700 rounded-lg"
                >
                  <X className="h-6 w-6" />
                </button>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-white">{selectedSubmission.organizationType}</p>
                    </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-white">{selectedSubmission.contactPerson}</p>
                    </div>
                    <div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-sm font-medium text-slate-400">Partnership Types</label>
                    ...
                    <div>
                      <label className="text-sm font-medium text-slate-400">Goals</label>
                      <p className="text-white">{selectedSubmission.goals}</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="text-sm font-medium text-slate-400">Budget Range</label>
                        <p className="text-white">{selectedSubmission.budget || 'Not specified'}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-slate-400">Timeline</label>
                        <p className="text-white">{selectedSubmission.timeline || 'Not specified'}</p>
                      </div>
                    </div>
                    {selectedSubmission.additionalInfo && (
                      <div>
                        <label className="text-sm font-medium text-slate-400">Additional Information</label>
                        <p className="text-white">{selectedSubmission.additionalInfo}</p>
                      </div>
                    )}
                  </div>
                </div>
